 ...
 * this kind of parsing, so that it can pass around a single "Context"
 * value instead of an octstr and one or more offset and length
 * parameters.
 *
...
/*
 * Return a Context object for this octstr, with parsing starting
 * at position 0 and the limit at the end of the string.
 */
Context *parse_context_create(Octstr *str);

/*
 * Destroy a Context object.  Note that this does not free the string
 * that was parsed.
 */
