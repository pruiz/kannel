...
WsBool
ws_stream_getc(WsStream *stream, WsUInt32 *ch_return)
{
...
void
ws_stream_ungetc(WsStream *stream, WsUInt32 ch)
{
  stream->ungetch = ch;
...
WsBool
ws_stream_flush(WsStream *stream)
{
  if (stream->flush)
...
void
ws_stream_close(WsStream *stream)
{
  if (stream->close)
...
WsStream *
ws_stream_new(void *context, WsStreamIOProc io, WsStreamFlushProc flush,
