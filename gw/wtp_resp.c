...
static void handle_wrong_version(WAPEvent *event){
       ...
       if (event->type == RcvInvoke)
          wtp_send_abort(PROVIDER, WTPVERSIONZERO, event->u.RcvInvoke.tid, 
...
static void handle_no_sar(WAPEvent *event){
       ...
       if (event->type == RcvInvoke)
          wtp_send_abort(PROVIDER, NOTIMPLEMENTEDSAR, 
                        event->u.RcvInvoke.tid, event->u.RcvInvoke.addr_tuple);
}
       ...
       if (event->type == RcvInvoke){
	     ...
	     handle_wrong_version(event);
          ...
          if (!event->u.RcvInvoke.ttr || !event->u.RcvInvoke.gtr){
             debug("wap.wtp_resp", 0, "WTP_RESP: no sar implemented, aborting"
                   "transaction");
             handle_no_sar(event);
          }
       }
 ...
 */
	              case RcvErrorPDU:
			   debug("wap.wtp_resp", 0, "an erronous pdu"
                                 "received");
                           wap_event_dump(event);
		      ...
		      break;
                      ...
                      break;
/*
 * This and the following branch represent test nro 3 in WTP 10.2.
 */
	              case RcvAck: 
			   ...
			   info(0, "WTP_RESP: resp_machine_find_or_create:"
	              ...
	              case TR_Invoke_Res: case TR_Result_Req: 
                      case TR_Abort_Req:
			   error(0, "WTP_RESP: resp_machine_find_or_create:"
                                " WSP primitive to a wrong WTP machine");
	              break;

	              case TimerTO_A: case TimerTO_R: case TimerTO_W:
                           error(0, "WTP_RESP: resp_machine_find_or_create: "
				 "timer event without corresponding machine");
		      break;
...
static WTPRespMachine *resp_machine_create(WAPAddrTuple *tuple, long tid, 
	...
	WTPRespMachine *resp_machine;
        ...
        #define WSP_EVENT(name) resp_machine->name = NULL;
        #define INTEGER(name) resp_machine->name = 0; 
        #define MSG(name) resp_machine->name = msg_create(wdp_datagram); 
...
static void resp_machine_destroy(void * p){
	...
	list_delete_equal(resp_machines, resp_machine);
        ...
        #define ENUM(name) resp_machine->name = LISTEN;
        #define WSP_EVENT(name) wap_event_destroy(resp_machine->name);
        #define INTEGER(name) resp_machine->name = 0; 
        #define MSG(name) msg_destroy(resp_machine->name); 
