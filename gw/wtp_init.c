/*
 * wtp_init.c - WTP iniator implementation
 *
 ...
 *
 * List of iniator WTP machines
 */

static List *init_machines = NULL;

/*
 * Counter for iniator WTP machine id numbers, to make sure they are unique.
 */
static Counter *init_machine_id_counter = NULL;

/*
 * Counter for GenTID (The global tid generated by iniator, see WTP 10.4.3). 
 */

static Counter *gen_tid_counter = NULL;

/*
 * We must set tid_new flag 1, when we are restarting the iniator. Otherwise
 * result could be lots of validations (see WTP 8.8.3.2). This counter supply
 * the iniatial value.
 */ 

static Counter *tid_new_counter = NULL;

/*
 * Queue of events to be handled by WTP iniator.
 */
static List *queue = NULL;

/*
 * Give the status of the wtp iniator:
 *
 *	limbo
 *		not running at all
 *	running
 ...
 */
static enum { limbo, running, terminating } iniator_run_status = limbo;
...
/*
 * Create and destroy an uniniatilised wtp iniator state machine
 */
...
/*
 * Checks whether wtp iniator machines data structure includes a specific 
 * machine.
 ...
 * an iniator, GenTID = SendTID (WTP 10.5). 
 */
static unsigned short rcv_tid(unsigned short tid);
static void start_iniator_timer_R(WTPInitMachine *machine); 
static void stop_iniator_timer(Timer *timer);
...
void wtp_iniator_init(void) {
     ...
     gen_tid_counter = counter_create();
     tid_new_counter = counter_create();
     ret = counter_set(tid_new_counter, 1);
     
     queue = list_create();
     list_add_producer(queue);

     gw_assert(iniator_run_status == limbo);
     iniator_run_status = running;
     gwthread_create(main_thread, NULL);
}

void wtp_iniator_shutdown(void) {
     gw_assert(iniator_run_status == running);
     iniator_run_status = terminating;
     list_remove_producer(queue);
     gwthread_join_every(main_thread);

     debug("wap.wtp", 0, "wtp_iniator_shutdown: %ld init_machines left",
     ...
     counter_destroy(gen_tid_counter);
     counter_destroy(tid_new_counter);
}
...
static void main_thread(void *arg) {
	...
	while (iniator_run_status == running && 
...
static WTPInitMachine *init_machine_create(WAPAddrTuple *tuple, unsigned short
       ...
       #define ENUM(name) init_machine->name = INIATOR_NULL_STATE;
...
/*
 * Give the name of an iniator state in a readable form. 
 */
...
/*
 * Feed an event to a WTP iniator state machine. Handle all errors by itself, 
 * do not report them to the caller. WSP indication or conformation is 
 * handled by an included state table. Note: Do not put {}s of the else block 
...
static void handle_init_event(WTPInitMachine *init_machine, WAPEvent *event){
       ...
       debug("wap.wtp", 0, "WTP_INIT: iniator machine %ld, state %s,"
             " event %s.", 
	     init_machine->mid, 
       ...
       #define ROW(init_state, event_type, condition, action, next_state) \
		  ...
		  debug("wap.wtp", 0, "WTP %ld: New state %s", init_machine->mid, #next_state); \
	      } else 
      #include "wtp_init_state-decl.h"
	      {
      ...
      if (init_machine->state == INIATOR_NULL_STATE)
...
static WTPInitMachine *init_machine_find_or_create(WAPEvent *event){
       ...
       switch (event->type){
	      ...
	      break;
/*
 * When we are receiving an invoke requirement, we must create a new 
 * transaction and a new tid. This can be wrapped, and should have first bit
 * turned.
 */
              case TR_Invoke_Req:
                   old_tid = (unsigned short) counter_value(gen_tid_counter);
		   tid = (unsigned short) counter_increase(gen_tid_counter);
	      ...
	      break;
              ...
              default:
		   error(1, "WTP_INIT: machine_find_or_create, unhandled"
                   "event");
                   wap_event_dump(event);
                   return NULL;
       }
	  ...
	  switch (event->type){
	         ...
	         case RcvAck:      
/* Case nro 2 If we do not have a tid asked for, we send
   a negative answer, i.e. an abort with reason INVALIDTID. */
...
/*
 * Creates TR-Abort.ind event from an iniator state machine. 
 */
static WAPEvent *create_tr_abort_ind(WTPInitMachine *sm, long abort_reason) {
	WAPEvent *event;
